...
class CategoryViewModel @Inject constructor (private val getFromAllJokesUseCase: GetFromAllJokesUseCase,
                                             @Named(SCHEDULER_IO) val subscribeOnScheduler: Scheduler,
                                             @Named(SCHEDULER_MAIN_THREAD) val observeOnScheduler: Scheduler)
    : ViewModel(){

    data class CategoryItem(val id: Int, val text: String, val userVote: Int, val overallRating: Float)

    val liveDataState =  MutableLiveData<State<CategoryViewModel.CategoryItem>>()
    ...
    private fun onJokeListReceived(jokeList: List<Joke>) {
        ...
        val jokeItemList = jokeList.map {
            CategoryViewModel.CategoryItem(id = it.id, text = it.text,
